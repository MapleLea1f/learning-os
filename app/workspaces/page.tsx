...
export default function WorkspacesPage() {
  ...
  const [formOpen, setFormOpen] = useState(() => typeof window !== "undefined" && Boolean(new URLSearchParams(window.location.search).get("plan")));
  const [name, setName] = useState("");
  const [description, setDescription] = useState("");
  const [localPath, setLocalPath] = useState("");
  const [planId, setPlanId] = useState(() => typeof window !== "undefined" ? new URLSearchParams(window.location.search).get("plan") ?? "" : "");
  ...
  async function createWorkspace(event: FormEvent) {
    ...
    if (error || !data) {
      ...
      if (linkError) {
        ...
        window.location.assign(`/workspace/${created.id}?plan=${planId}`);
      }
    }
    setSaving(false);
  }
