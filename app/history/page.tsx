    ...
    : loading
      ...
      : <section className="daily-records">
          {visibleRecords.length ? visibleRecords.map((record) => {
            const events = normalizeEvents(record.events, record).filter((event) => event.committed === true);
            const evidence = normalizeEvidence(record.evidence_json ?? record.evidence).filter((item) => item.committed === true);
